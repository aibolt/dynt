...
Formation uses event-driven CSS transitions and retains no animation loop. Its optional viewport flow creates four CSS-animated strokes only while a target is being approached, bounds the full-set delay window to 1800 milliseconds, and removes every transient flight after completion. Destroying or withdrawing cancels pending flow timers and removes their nodes. Kinetic removes a surface from scheduling after its pressure, tilt, drift, wave, and impact channels reach rest. Pausing or destroying a controller cancels the pending frame and clears its active set.
